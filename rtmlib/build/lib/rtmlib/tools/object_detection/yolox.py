...
from ..base import BaseTool, YOLOXTensorRT
...
#         #     print(imgs.shape)
#         #     outputs = self.batch_inference(imgs)[0]
#         #     print(outputs)
...
#             outputs = outputs.reshape(-1)[:50].reshape(1,-1,5)
...
class YOLOX(YOLOXTensorRT):
    ...
    def __call__(self, image):
        ...
        outputs = self.inference(imgs)
        res_box, res_scores = self.postprocess(outputs, ratios)
        if type(image) is list or type(image) is tuple:
            return res_box, res_scores
        else:
            if len(res_box):
                return res_box[0], res_scores[0]
    ...
    def postprocess(
        ...
        pack_dets = (outputs[:, :, :4], outputs[:, :, 4])
        final_boxes, final_scores = pack_dets
        final_boxes /= np.array(ratios).reshape(-1,1,1)

        res_box = []
        res_scores = []
        for i in range(len(final_boxes)):
            isscore = final_scores[i] > 0.3
            isbbox = [j for j in isscore]
            bboxes = final_boxes[i][isbbox]
            scores = final_scores[i][isbbox]
            res_box.append(bboxes)
