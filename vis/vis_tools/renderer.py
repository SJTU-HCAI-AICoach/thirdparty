...
class Renderer():
    ...
    def render_with_ground(self, verts, faces, colors, cameras, lights, pointcloud = None):
        """
        :param verts (B, V, 3)
        :param faces (F, 3)
        :param colors (B, 3)
        ...
        images_mesh = self.renderer(mesh, cameras=cameras, lights=lights, materials=materials)
        if pointcloud is not None:
            # pointcloud (n, 3)
            pointcloud = Pointclouds(points=[pointcloud], features=[torch.tensor([[0,1,0]]).repeat(pointcloud.size(0), 1).to(pointcloud)])
            images_pointcloud = self.point_renderer(pointcloud, cameras = cameras, lights = lights, materials=materials)
            combined_images = torch.clamp(images_mesh[..., :3] + images_pointcloud[..., :3], 0, 1)

        else:
            combined_images = images_mesh
        image = (combined_images[0, ..., :3].cpu().numpy() * 255).astype(np.uint8)
            
        return image
    def render_pc_with_ground(self, cameras, lights, pointcloud = None):
        """
        :param verts (B, V, 3)
        :param faces (F, 3)
        :param colors (B, 3)
